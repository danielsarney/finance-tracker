...
class IncomeModelTest(TestCase):
    ...
    def test_income_ordering(self):
        ...
        for i in range(len(ordered_incomes) - 1):
            self.assertGreaterEqual(
                ordered_incomes[i].date, ordered_incomes[i + 1].date
            )
    ...
    def test_income_is_taxable_default(self):
        ...
        new_income = IncomeFactory(user=self.user, category=self.category)
    ...
    def test_income_is_taxable_custom(self):
        ...
        new_income = IncomeFactory(
            user=self.user, category=self.category, is_taxable=False
        )
...
class IncomeViewsTest(TestCase):
    ...
    def test_income_list_view_filtering(self):
        ...
        response = self.client.get(
            f"{reverse('income:income_list')}?year={current_year}"
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(
            f"{reverse('income:income_list')}?month={current_month}"
        )
        self.assertEqual(response.status_code, 200)

        # Test category filtering
        response = self.client.get(
            f"{reverse('income:income_list')}?category={self.category.id}"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_income_create_view_post_valid_data(self):
        ...
        new_income = Income.objects.filter(
            user=self.user, amount=Decimal("5000.00"), payer="New Company"
        ).first()
    ...
    def test_income_detail_view_requires_login(self):
        ...
        response = self.client.get(
            reverse("income:income_detail", kwargs={"pk": self.income.pk})
        )
        self.assertEqual(response.status_code, 302)
        self.assertIn("/accounts/login/", response.url)
    ...
    def test_income_detail_view_other_user_income(self):
        ...
        self.client.force_login(self.user)
        response = self.client.get(
            reverse("income:income_detail", kwargs={"pk": self.other_income.pk})
        )
    ...
    def test_income_update_view_post_valid_data(self):
        ...
        response = self.client.post(
            reverse("income:income_update", kwargs={"pk": self.income.pk}), form_data
        )
    ...
    def test_income_delete_view_requires_login(self):
        ...
        response = self.client.get(
            reverse("income:income_delete", kwargs={"pk": self.income.pk})
        )
        self.assertEqual(response.status_code, 302)
        self.assertIn("/accounts/login/", response.url)
...
class IncomeIntegrationTest(TestCase):
    ...
    def test_complete_income_workflow(self):
        ...
        new_income = Income.objects.filter(
            user=self.user, amount=Decimal("8000.00"), payer="Test Company"
        ).first()
        self.assertIsNotNone(new_income)
        self.assertTrue(new_income.is_taxable)
        ...
        update_response = self.client.post(
            reverse("income:income_update", kwargs={"pk": new_income.pk}), update_data
        )
    ...
    def test_income_taxable_field_behavior(self):
        ...
        taxable_income = Income.objects.filter(
            user=self.user, payer="Taxable Company"
        ).first()
        ...
        response = self.client.post(
            reverse("income:income_create"), form_data_non_taxable
        )
        self.assertEqual(response.status_code, 302)

        non_taxable_income = Income.objects.filter(
            user=self.user, payer="Non-Taxable Company"
        ).first()
