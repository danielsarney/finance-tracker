...
class WorkLogForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'company_client': forms.Select(attrs={'class': 'form-control', 'id': 'id_company_client'}),
            'hours_worked': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.25', 'min': '0'}),
            'hourly_rate': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.01', 'min': '0', 'id': 'id_hourly_rate'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user:
            self.fields['company_client'].queryset = Client.objects.filter(user=user).order_by('company_name')
