...
class MileageLog(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="mileage_logs"
    )
    date = models.DateField(help_text="Date of the journey")

    # Journey details
    start_location = models.CharField(
        max_length=200, help_text="Starting location (e.g., Home Office)"
    )
    end_location = models.CharField(
        max_length=200, help_text="Destination (e.g., Client Office)"
    )
    purpose = models.CharField(
        max_length=200, help_text="Business purpose of the journey"
    )
    ...
    miles = models.DecimalField(
        max_digits=6, decimal_places=1, help_text="Number of miles driven"
    )
    ...
    def calculate_claim(self):
        ...
        existing_miles = MileageLog.objects.filter(
            user=self.user, date__gte=tax_year_start, date__lte=tax_year_end
        ).exclude(pk=self.pk).aggregate(total=models.Sum("miles"))["total"] or Decimal(
            "0"
        )
        ...
        if total_miles_in_year <= 10000:
            ...
            self.total_claim = (miles_at_45p * Decimal("0.45")) + (
                miles_at_25p * Decimal("0.25")
            )
        else:
